...
function extrair(texto: string): Array<{ dataRaw: string; descricaoRaw: string; valorRaw: string }> {
    ...
    const isBradesco = /bradesco/i.test(limpo.substring(0, 3000));
    ...
    function combinarDescricao(buffer: string, atual: string): string {
        ...
        const atualIniciaNova = BRADESCO_NOVA_TRANSACAO_RE.test(atualTrim);
    ...
    for (let i = 0; i < linhas.length; i++) {
        ...
        if (mData) {
            ...
            if (mValorProximo && descSemData.length > 0 && !/^saldo\s+/i.test(descSemData)) {
                ...
                let descPura = combinarDescricao(descAcumulada, descSemData);
            ...
            const novaAcumuladaData = combinarDescricao(descAcumulada, descSemData);
            ...
            if (valoresMatches.length > 0) {
                ...
                const novaAcumuladaElse = combinarDescricao(descAcumulada, linha);
