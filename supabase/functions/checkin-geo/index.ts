...
Deno.serve(async (req: Request) => {
  ...
  // ── 6. Janela de horário: 08:00–14:30 BRT ────────────────────────────────
  const brtMinutes = getBRTMinutes();
  if (brtMinutes < (8 * 60) || brtMinutes > (14 * 60 + 30)) {
    return json({ error: 'fora_do_horario', message: 'Check-in permitido apenas entre 08:00 e 14:30.', brt_minutes: brtMinutes }, 403);
  }
