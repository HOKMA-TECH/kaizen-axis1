...
// Full simulation of extrair() with Revolut text
...
// From debug_texto_extraido - this is the ACTUAL text received by the API (with pipes)
...
// Step 1: Normalization (same as code)
...
// Data regex from the code
const DATA_RE = /^(\d{2}[\/\-\.\s]\d{2}(?:[\/\-\.\s]\d{4})?|\d{2}[\/\-\.\s]+(?:JAN|FEV|MAR|ABR|MAI|JUN|JUL|AGO|SET|OUT|NOV|DEZ)[\/\-\.\s]?(?:\d{2,4})?)/i;
const VALOR_RE_LINHA = /([+-]?\s*(?:R\$?\s*|[A-Z]{1,3}\$?\s*)?\d{1,3}(?:[.,]\d{3})*[.,]\d{2})(?:\s*([CD]|\(\+\)|\(-\)|\+|-))?(?=\s|$|\||[A-Za-z])/ig;

let dateMatchCount = 0;
let valorMatchCount = 0;
...
linhas.forEach((linha, i) => {
    ...
    if (mData) {
        dateMatchCount++;
        const descSemData = linhaProcessada.substring(mData[0].length).trim();
        const valoresLine = Array.from(descSemData.matchAll(VALOR_RE_LINHA));
        console.log(`\n[LINHA ${i}] Data=${mData[1]} desc="${descSemData.substring(0,50)}" valores=${valoresLine.map(m=>m[1]).join(',')}`);
        if (valoresLine.length > 0) valorMatchCount++;
    }
});

console.log(`\nData matches: ${dateMatchCount}, Com valores: ${valorMatchCount}`);
