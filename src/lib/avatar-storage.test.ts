...
import { buildChatAvatarPath, buildSettingsAvatarPath, getAvatarExtension, isAllowedAvatarMimeType } from './avatar-storage';
...
assert.equal(
  buildSettingsAvatarPath('user-123', { name: 'foto.jpeg', type: 'image/jpeg' }),
  'user-123/avatar.jpg',
  'settings avatar path must stay stable so upsert reuses the same object',
);
